  ...
  namespace loki {
    ...
    worker_t::result_t loki_worker_t::route(boost::property_tree::ptree& request, http_request_t::info_t& request_info) {
      ...
      for(size_t i = 0; i < locations.size(); ++i) {
        ...
        request.put_child("correlated_" + std::to_string(i), correlated.rbegin()->second.ToPtree(i));
        //TODO: get transit level for transit costing
        //TODO: if transit send a non zero radius
        auto colors = connectivity_map.get_colors(reader.GetTileHierarchy().levels().rbegin()->first, correlated.rbegin()->second, 0);
